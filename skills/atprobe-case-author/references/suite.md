...
- 按顺序执行 `cases` 引用的用例。
- **断连阀（suite_setup 失败语义）**：`suite_setup` 任一步 FAIL（默认 abort 策略），或步骤
  失败原因是**断连**（DISCONNECT——即使该步配了 `on_failure: continue/skip`）→ 立即终止
  suite_setup，**全部 cases 跳过**（记 SKIPPED），`suite_teardown` 仍执行，`run` 退出码为
  **1**（套件前置没跑成是真实失败，v0.10 起不再被全零 summary 掩盖）。断连阀是防止向已断开
  端口继续发剩余 setup 步骤（每步拖满超时、白烧 n×步骤超时）。
