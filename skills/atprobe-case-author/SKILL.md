...
- 业务码响应步骤都加了 `timeout`
- 两阶段用例：expect 步骤后**紧跟 data 步骤**，且 expect/wait_urc 都配了断言（尾锚 `\r\n> $` 形态，见「两阶段发送三陷阱」）
...
- **等待期收到错误码行立即终结**：ERROR / +CME ERROR / +CMS ERROR 在 wait_urc 模式下
  不再等到超时——设备已明确拒绝，继续等目标 URC 只会烧完超时预算。
...
### 两阶段发送三陷阱（expect + data；2026-08-29 真机实证固化）

两阶段指令（FSWF/TCPSEND/CIPSEND 类：阶段一声明长度出 `>` 提示符，阶段二发数据）用
`expect` 声明阶段一完成形态、紧跟 `data` 步骤发内容。三个隐蔽陷阱，工具已内置对应告警
（告警不改变执行，看到即修）：

1. **expect 命中后必须紧跟 data 步骤**。设备收到提示符即进入等数据态——若 data 步漏写、
   被 when 跳过、或在 data 之前中止（如 expect 步骤断言失败 abort），后续所有 AT 命令的
   字节会被设备**当作数据静默吞掉**（真机实证：teardown 命令前 16 字节被 FSWF 会话吞）。
   解析期「直接后继不是 data 步骤」与运行期「两阶段悬置」双重告警。
2. **expect/wait_urc 必配严格断言**。expect 单独使用不是严格闸门：超时回退业务码路径
   （TIMEOUT 三态：文本完整以 \r\n 结尾 → 走正常断言），无断言则**无条件放行**（假 PASS
   实证）。建议尾锚断言：`assert: { matches: '\r\n> $' }`。
3. **断言文本止于 expect 匹配点**（命中即截断）——提示符之后的尾随字节不进断言文本；
   尾锚 `$` 可用，**勿用首锚 `^`**（设备可能回显命令行，回显行在提示符之前）。

配套写法：阶段一步骤可加 `interval`（提示符后延迟 50-100ms 再发数据，CIPSEND 实测形态）；
「指令声明长度 = 数据长度」用 `{{file_size("路径")}}` 渲染文件字节数；数据内容走 `data`
（file/inline/inline_hex，发送的是内容字节流）。schema 细节见 `references/yaml-schema.md`
「expect」「数据流输入」两节。
