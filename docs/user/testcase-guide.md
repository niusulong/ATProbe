...
**单行业务码的提取正则用排除字符类 `([^\r\n]+)`，不要用贪婪 `.+`**：响应文本
逐行保留 `\r`（框架不做行尾剥除），`(.+)` 会连同行尾 `\r` 一起吞入变量——后续
...
### 12.7 两阶段发送的三个陷阱（2026-08-29 真机实证固化）

两阶段指令（FSWF/TCPSEND/CIPSEND 类：阶段一声明长度出 `>` 提示符，阶段二发数据）有三个
隐蔽陷阱，工具已内置对应告警（告警不改变执行，仍需用例作者修正）：
...
1. **expect 命中后必须紧跟 data 步骤**。设备收到提示符即进入等数据态——若 data 步漏写、
   ...
   解析期"直接后继不是 data 步骤"与运行期"两阶段悬置"双重告警，看到即修。
...
3. **断言文本止于 expect 匹配点**（命中即截断）——提示符之后的尾随字节不进断言文本；
   因此尾锚 `$` 可用，但**不要用首锚 `^`**（设备可能回显命令行，回显行在提示符之前）。
