...
- 值非法（如 `step_timeout: abc`、`ports` 非列表、帧格式非 3 字符）→
...
| 第 3 段 | 帧格式（3 字符紧凑写法） | 可省略或留空 → `8N1`；非法报「帧格式无效」 |
...
### 6.1 帧格式（ports 第 3 段，3 字符紧凑写法）

| 位 | 合法值 | 默认 | 说明 |
|---|---|---|---|
| 数据位 | `5` / `6` / `7` / `8` | `8` | 首字符必须是这四个数字之一 |
| 校验位 | `N` / `E` / `O` / `M` / `S` | `N` | 无/偶/奇/Mark/Space；大小写不敏感 |
| 停止位 | `1` / `2` | `1` | 模型另支持 1.5，但 3 字符紧凑写法无法表达（`8N1.5` 因长度非 3 报错） |

合法示例：`8N1`（默认）、`7E1`、`8O2`、`5S1`。
