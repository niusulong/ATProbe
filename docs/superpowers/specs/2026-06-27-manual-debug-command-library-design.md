...
> **架构修订（2026-06-27，实现后）**：本文档原设计为「主窗口右侧全局 QDockWidget 停靠面板」。
> 实现后用户反馈命令库仅在手动调试页使用，全局 Dock 过重，故**改为手动调试页内嵌侧栏**：
> - `CommandLibraryDock` → `CommandLibraryPanel`（普通 QWidget，嵌入手动调试页 QSplitter 左侧）
> - 双击命令直接页内 `send_command`（不再经主窗口 `_on_command_send` 路由）
> - 移除 `mainwindow._init_command_dock` / `_on_command_send`
> - 管理对话框顶部工具栏始终可见新增入口 `[＋项目][＋功能组][＋命令][加载][另存为]`，根据当前选中节点智能定位目标
> 下文 §2/§5 仍保留原始设计描述作为决策记录，以最新实现（代码）为准。
